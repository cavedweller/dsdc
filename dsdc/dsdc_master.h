...
class dsdcm_slave_base_t : public aclnt_wrap_t {
  public:
    ...
    void
    get_xdr_repr(dsdcx_slave_t* o) {
        *o = _xdr_repr;
    }
    const str&
    remote_peer_id() const {
        return _client->remote_peer_id();
    }
    ptr<aclnt>
    get_aclnt() {
        return _clnt_to_slave;
    }
    void
    handle_heartbeat() {
        _last_heartbeat = sfs_get_timenow();
    }
...
template <>
struct keyfn<dsdcm_slave_t, str> {
    keyfn() {}
    const str&
    operator()(dsdcm_slave_t* s) const {
...
class dsdc_master_t : public dsdc_app_t {
  public:
    ...
    void
    insert_node(dsdc_ring_node_t* node) {
        _hash_ring.insert(node);
    }
    void
    remove_node(dsdc_ring_node_t* node) {
        _hash_ring.remove(node);
    }

    void
    insert_client(dsdcm_client_t* cli) {
        _clients.insert_head(cli);
    }
    void
    remove_client(dsdcm_client_t* cli) {
        _clients.remove(cli);
    }
    ...
    void
    insert_lock_node(dsdc_ring_node_t* node) {}
    void
    remove_lock_node(dsdc_ring_node_t* node) {}
    ...
    void
    broadcast_newnode(const dsdcx_slave_t& x, dsdcm_slave_t* skip, CLOSURE);
