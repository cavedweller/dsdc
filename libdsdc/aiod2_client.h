...
struct remote_t {
    ...
    str
    to_str() {
        return strbuf("%s:%d", m_host.cstr(), m_port);
    }
    str m_host;
    int m_port;
};
...
class remote_client_t : public base_client_t {
  public:
    remote_client_t(mgr_t* m, remote_t r) : base_client_t(m), m_remote(r) {}
    ~remote_client_t() {}
    void launch(evb_t ev, CLOSURE);
...
class mgr_t : public virtual refcount {
  ...
  private:
    void get_ready(event<int, ptr<base_client_t>, ptr<aclnt>>::ref ev, CLOSURE);
