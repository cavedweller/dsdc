...
class dsdcl_mgr_t {
  ...
  private:
    ...
    dsdc_lock_t*
    find_lock(const dsdc_key_t& k) {
        return _locks[k];
    }
    void
    insert(dsdc_lock_t* l) {
        _locks.insert(l);
    }
    void
    remove(dsdc_lock_t* l) {
        _locks.remove(l);
    }

    ihash<dsdc_key_t, dsdc_lock_t, &dsdc_lock_t::_key, &dsdc_lock_t::_hlnk>
        _locks;
};
