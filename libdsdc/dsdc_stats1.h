    ...
    struct dataset_t {
        dataset_t(bool per_epoch)
            : _lifetime(per_epoch ? NULL : New histogram_t()),
              _n_active(per_epoch ? NULL : New int()), _per_epoch(per_epoch) {
            clear();
        }
        void clear();
        ~dataset_t() {
...
namespace annotation {
    ...
    class base1_t : public base_t {
      public:
        ...
        void
        missed_get() {
            _alltime._missed_gets++;
            _per_epoch._missed_gets++;
        }
        void
        missed_remove() {
            _alltime._missed_removes++;
            _per_epoch._missed_removes++;
        }
        void
        put() {
            _alltime._puts++;
        ...
        void
        inc_n_active() {
            _alltime.inc_n_active();
            _per_epoch.inc_n_active();
        }

        void
        dead_object_n_gets(int n) {
            _alltime._do_gets.add(n);
            _per_epoch._do_gets.add(n);
        }
        void
        time_alive(int n) {
            _alltime._lifetime->add(n);
        }
        void
        dead_object_time_alive(int n) {
            _alltime._do_lifetime.add(n);
            _per_epoch._do_lifetime.add(n);
        }
        void
        objsz(int n) {
            _alltime._objsz.add(n);
            _per_epoch._objsz.add(n);
        }
        void
        dead_object_objsz(int n) {
            _alltime._do_objsz.add(n);
            _per_epoch._do_objsz.add(n);
        }
    ...
    class int_t : public base1_t {
      public:
        ...
        bool
        to_xdr(dsdc_annotation_t* a) const {
