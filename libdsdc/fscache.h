...
class cfg_t {
  public:
    ...
    bool
    debug(u_int64_t u) const {
        return (u & _debug) == u;
    }
    void
    set_debug_flag(u_int64_t f) {
        _debug |= f;
    }
    time_t
    rollover_time() const {
    ...
    bool
    cache_on_load() const {
        return _cache_on_load;
    }
    ptr<aiod2::remotes_t>
    aiod2_remotes() const {
        return _aiod2_remotes;
    }
...
class file_id_t {
  public:
    file_id_t(const str& name, u_int32_t index) : _name(name), _index(index) {}
    file_id_t() {}
    str
    name() const {
        return _name;
...
class backend_t {
  public:
    ...
    virtual void
    str2file(str f, str s, int mode, evi_t cb) {
        str2file_T(f, s, mode, cb);
    }
    virtual void
    str2file_inner(str f, str s, int md, bool cf, evi_t cb) {
        assert(false);
    }
    ...
    virtual void
    stat(str f, struct stat* sb, evi_t ev) {}
    virtual void
    glob(str d, str f, vec<str>* out, evi_t ev) {
        ev->trigger(-EINVAL);
    }
...
class engine_t {
  public:
    ...
    virtual void
    load(file_id_t id, cbits_t cb) {
        load_T(id, cb);
    }
    virtual void
    store(file_id_t id, time_t tm, str data, evi_t ev) {
        store_T(id, tm, data, ev);
    }
    virtual void
    remove(file_id_t id, evi_t ev) {
        remove_T(id, ev);
    }
...
class write_delay_engine_t : public engine_t {
  public:
    ...
    virtual void
    init(evb_t ev) {
        write_delay_engine_t::init_T(ev);
    }
    virtual void
    shutdown(evv_t ev) {
        shutdown_T(ev);
    ...
    bool
    use_cache() const {
        return _cfg->write_delay() && m_enabled;
    }
...
class thread_backend_t : public backend_t {
  public:
    ...
    void
    file2str(str fn, evis_t cb) {
        file2str_T(fn, cb);
    }
    void
    str2file_inner(str f, str s, int m, bool cf, evi_t cb) {
        str2file_inner_T(f, s, m, cf, cb);
    }

    void
    remove(str f, evi_t cb) {
        remove_T(f, cb);
    }
    void
    mkdir(str f, int mode, evi_t ev) {
        mkdir_T(f, mode, ev);
    }
    void
    statvfs(str d, struct statvfs* buf, evi_t ev) {
        statvfs_T(d, buf, ev);
    }
    void init(evb_t ev);
    void
...
class aiod2_backend_t : public backend_t {
  public:
    ...
    void
    str2file_inner(str f, str s, int m, bool c, evi_t cb) {
  ...
  private:
    void file2str_T(str fn, evis_t cb, CLOSURE);
    void str2file_inner_T(str f, str s, int mode, bool c, evi_t cb, CLOSURE);
...
class aiod_backend_t : public backend_t {
  public:
    ...
    void
    str2file(str f, str s, int m, evi_t cb) {
        str2file_T(f, s, m, cb);
    }
...
class iface_t {
  public:
    ...
    void
    load(file_id_t id, load_cb_t cb) {
        _engine->load(id, wrap(this, &iface_t<C>::load_cb, id, cb));
    }

    void
    store(file_id_t id, time_t tm, const C& obj, evi_t ev) {
        str s;
        str fn = filename(id);
        int ret;
        if (!(s = xdr2str(obj))) {
    ...
    void
    remove(file_id_t id, evi_t cb) {
        _engine->remove(id, cb);
    }
    str
    filename(file_id_t id) const {
        return _engine->filename(id);
    }

    void
    statvfs(struct statvfs* buf, evi_t ev) {
        _engine->statvfs(buf, ev);
    }

    void
    set_engine(engine_t* e) {
        _engine = e;
    }

  private:
    void
    load_cb(file_id_t id, load_cb_t cb, int rc, time_t tm, str s) {
        ptr<C> ret;
        if (rc == 0) {
